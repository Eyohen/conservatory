...
const  SeeSpecial = () => {
  const navigate=useNavigate()
    ...
    //   }
        ...
        const res = await axios.get(URL+"/api/specials"
        //,{
        //   headers: {
    ...
    const handleDelete=async(itemId)=>{
      try{
        ...
        const res = await axios.delete(URL+"/api/specials/"+itemId,{
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        })
            ...
            {items.map((item) => (
              ...
              >
               ...
               <td class="px-6 py-2">{item.fullName}</td>
                ...
                <td class="px-6 py-2">{item.description.slice(0,9)}...</td>
                ...
                <Link to={`/specialitem/${item._id}`}><td class="px-6 py-2"><HiOutlineArchiveBox className='mt-3'/></td></Link>
