...
//       <input onChange={(e) => setGiftEmail(e.target.value)}  className='border border-gray-300 bg-[#F6EADF] text-gray-800 px-2 py-1 w-[350px]' placeholder='Enter Recipients email' />
//     </div>
  ...
  return (
    <div className="bg-[#F6EADF] min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8">
        <h1 className='text-2xl md:text-3xl text-[#5b3e31] text-center mb-6'>Our Tea Menus</h1>
        ...
        <div className='flex flex-col gap-4 max-w-[300px] mx-auto mb-6'>
          {timeslots.map((timeSlot) => (
            <button
              key={timeSlot}
        ...
        {availability && (
          <p className={`text-center ${availability === `This time is Available` ? 'text-green-600' : 'text-red-600'} mb-6`}>
            {availability}
          </p>
        )}
        ...
        {showCrockery && <p className='text-center text-lg md:text-xl mb-6'>Please select from our variety of Crockeries</p>}

        {showCrockery && selectedCrockery && (
          <div className="mb-6">
            <h2 className='text-xl md:text-2xl text-[#5b3e31] font-semibold text-center mb-4'>{selectedCrockery.crockery}</h2>
            ...
            <p className='text-lg md:text-xl text-[#5b3e31] text-center'>{selectedCrockery.description}</p>
          </div>
        )}
        ...
        {showTea && <p className='text-center text-lg md:text-xl mb-6'>Please select from our variety of tea</p>}

        {showTea && selectedMenu && (
          <div className="mb-6">
            <h2 className='text-xl md:text-2xl text-[#5b3e31] font-semibold text-center mb-4'>{selectedMenu.title}</h2>
            <div className='flex justify-center mb-4'>
              <img src={selectedMenu.imageUrl || tearoom} className='object-scale-down w-full max-w-[550px] h-auto' alt={selectedMenu.title} />
            </div>
            <p className='text-xl font-semibold text-center mb-4'>₦{numberFormatter.format(selectedMenu.price)}</p>
            <p className='text-lg md:text-xl text-[#5b3e31] text-center mb-4'>{selectedMenu.description}</p>

            <h3 className='text-xl md:text-2xl text-[#5b3e31] font-semibold text-center mb-2'>Savoury</h3>
            <p className='text-lg md:text-xl text-[#5b3e31] text-center mb-4'>{selectedMenu.savoury}</p>

            <h3 className='text-xl md:text-2xl text-[#5b3e31] font-semibold text-center mb-2'>Semi-Sweet</h3>
            <p className='text-lg md:text-xl text-[#5b3e31] text-center mb-4'>{selectedMenu.semiSweet}</p>

            <h3 className='text-xl md:text-2xl text-[#5b3e31] font-semibold text-center mb-2'>Dessert</h3>
            <p className='text-lg md:text-xl text-[#5b3e31] text-center'>{selectedMenu.dessert}</p>
          </div>
        )}

        <h2 className='text-xl md:text-2xl text-[#5b3e31] font-semibold text-center mb-4'>Beverages</h2>
        <p className='text-lg md:text-xl text-[#5b3e31] text-center mb-6'>Please select a minimum of two beverages</p>

        <h3 className='text-xl md:text-2xl text-[#5b3e31] font-semibold text-center mb-4'>Tea Selection</h3>
        ...
        <h3 className='text-xl md:text-2xl text-[#5b3e31] font-semibold text-center mb-2'>Coffee Selection</h3>
        <p className='text-lg md:text-xl text-[#5b3e31] text-center mb-4'>(additional ₦7,500)</p>

        <div className='mb-6'>
          <select value={selectedCoffee} onChange={handleCoffee} className="w-full max-w-[500px] mx-auto block border border-gray-300 bg-[#F6EADF] text-gray-500 px-4 py-2 rounded">
            <option value="">COFFEES:</option>
            {coffee.map(item => (
              <option key={item.id} value={item.coffee}>{item.coffee}</option>
            ))}
        ...
        <h3 className='text-xl md:text-2xl text-[#5b3e31] font-semibold text-center mb-2'>Ice Tea Selection</h3>
        <p className='text-lg md:text-xl text-[#5b3e31] text-center mb-4'>(additional ₦7,500)</p>

        <div className='mb-6'>
          <select value={selectedIceTea} onChange={handleIceTea} className="w-full max-w-[500px] mx-auto block border border-gray-300 bg-[#F6EADF] text-gray-500 px-4 py-2 rounded">
            <option value="">ICE TEAS:</option>
            {iceTea.map(item => (
              <option key={item.id} value={item.iceTea}>{item.iceTea}</option>
            ))}
        ...
        <div className='flex justify-center'>
          {isAuthenticated ? (
            <PaystackButton {...paymentProps} disabled={!canBook} className="bg-[#5b3e31] text-white py-2 px-6 rounded text-base md:text-lg">
              {canBook ? "Make Booking" : "Select at least 2 beverages"}
            </PaystackButton>
          ) : (
            <button onClick={() => navigate('/login')} className="bg-[#ffb640] text-white py-2 px-6 rounded text-base md:text-lg">
